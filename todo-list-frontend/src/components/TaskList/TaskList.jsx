...
const TaskList = ({ tasks, onTaskUpdated, onTaskDeleted }) => {
  ...
  // Muestra una alerta con tipo y mensaje
  ...
  // Abre el modal con una tarea, opcionalmente en modo edición
  ...
  // Alterna entre modo vista y edición en el modal
  ...
  // Actualiza una tarea enviándola al backend
        ...
        >
          <div className="flex items-center space-x-3">
            <input
              ...
              className="h-5 w-5 text-purple-800 rounded focus:ring-blue-500"
            />
            <h3
              onClick={() => openModal(task)}
              className={`text-lg cursor-pointer hover:text-purple-600 transition-colors ${task.completed ? 'line-through text-purple-900' : 'text-gray-800'}`}
            >
              {task.title}
            </h3>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
            ...
            {modalState.isEditing ? (
              <form onSubmit={handleUpdateTask} className="space-y-4">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="h-4 w-4 text-purple-800 rounded focus:ring-purple-500"
                  />
                ...
                <div className="mt-6 flex justify-end space-x-3">
                  ...
                  <button type="submit" className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-800">Guardar</button>
                </div>
              </form>
            ) : (
              <div className="space-y-3">
                ...
                <div className="mt-6 flex justify-end space-x-3">
                  <button onClick={toggleEditMode} className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-800">Editar</button>
                </div>
              </div>
            )}
          </div>
